...
class _ExternalPageState extends State<ExternalPage> {
  ...
  ExternalTaskModel formModel = ExternalTaskModel();
...
Future<Null> getProjectName() async {
  ...
  Future<Null> submitTask(ExternalTaskModel formData) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.addExternalTask(formData).then((value) {
        String status = 'fail please try again';
        if (value != null) {
          status = 'successful';
          apiService
              .addExternalImage(formData, value["IntTaskId"])
              .then((value) {
            status = 'successful';
          });
        }
  ...
  Future<Null> getTaskByDate(ExternalTaskModel formData, DateTime date) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.getExternalTask(formData, date).then((value) {
  ...
  //updateConsturcTask
  Future<Null> updateTask(ExternalTaskModel formData, DateTime date) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.updateExternalTask(formData, date).then((value) {
        String status = 'fail please try again';
        if (value != null) {
          print(value);
          status = 'successful';
