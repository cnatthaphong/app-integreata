...
class DeflectPage extends StatefulWidget {
  const DeflectPage({Key? key}) : super(key: key);

  @override
  State<DeflectPage> createState() => _DeflectPageState();
}

class _DeflectPageState extends State<DeflectPage> {
  DateTime date = DateTime.now();
  final ImagePicker imgpicker = ImagePicker();
  DeflectTaskModel formModel = DeflectTaskModel();
                                        ...
                                        children: [
                                          ...
                                          Padding(
                                            ...
                                            child: dropdownCustom(floorsOption,
                                              ...
                                              getRooms(formModel.IntBuildingId,
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              Padding(
                                                ...
                                                child: dropdownCustom(
                                                    LoopsOption,
                                                    formModel.IntRoomId.toString(),
                                                    (newValue) {
                                                  setState(() {
                                                    formModel.setIntRoomId(
                                                        int.parse(newValue));
                                                  });
                              ...
                              if (formModel.listImages != [])
                               ...
                               if (fileNames.length > 0)
                                Wrap(
                                  children: 
                                  fileNames.map((name) {
                                    String path = 'https://m.integreata.com/uploads/project/'+projectGuId+'/Deflect/'+DeflectId.toString()+'/'+name['FileName'];
                                    return Container(
                                        child: Card(
                                      child: Container(
                                        child: Stack(
...
Future<Null> getProjectName() async {
  ...
  Future<Null> getRooms(int IntBuildingId, int IntFloorId) async {
    try {
      setState(() {
        formModel.setIntRoomId(-1);
        LoopsOption = [
          DropdownMenuItem(child: Text("No choose"), value: '-1'),
        ];
      });
      if (IntBuildingId != -1 || IntFloorId != -1) {
        ...
        apiService.getRooms(IntBuildingId, IntFloorId).then((value) {
          if (value != null) {
            setState(() {
              for (var room in value['rooms']) {
                String IntRoomId = room['IntRoomId'].toString();
                String RoomNo = room['RoomNo'].toString();
                if (RoomNo.length > 12) {
                  RoomNo = RoomNo.substring(0, 11) + "...";
                }
                LoopsOption.add(
                    DropdownMenuItem(child: Text(RoomNo), value: IntRoomId));
              }
            });
          }
          setState(() {
  ...
  Future<Null> submitTask(DeflectTaskModel formData) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.addDeflectTask(formData).then((value) {
        String status = 'fail please try again';
        if (value != null) {
          status = 'successful';
          apiService
              .addDeflectImage(formData, value["IntTaskId"])
              .then((value) {
            status = 'successful';
          });
        }
  ...
  Future<Null> getTaskByDate(DeflectTaskModel formData, DateTime date) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.getDeflectTask (formData, date).then((value) {
        if (value != null && value['status'] == "have") {
          setState(() {
            formModel.setHoldStatusId(value['HoldStatusId']);
            formModel.setTaskStatusId(value['DeflectStatusId']);
            formModel.setTaskDescription(value['DeflectDescription']);
            fileNames = value['filename'];
            DeflectId = value['IntDeflectId'];
            isEdit = true;
          });
        } else {
          setState(() {
  ...
  //updateConsturcTask
  Future<Null> updateTask(DeflectTaskModel formData, DateTime date) async {
    try {
      setState(() {
        isApiCallProcess = true;
      });
      apiService.updateDeflectTask(formData, date).then((value) {
        String status = 'fail please try again';
        if (value != null) {
          status = 'successful';
        }
